...
class ValidationTest {
    ...
    void testAdvanceValidate() {
        ...
        assertThat(result1.isEmpty()).isEqualTo(expected1);

        Address address2 = new Address("EU", "London", "1-st street", "5", "1");
        Map<String, List<String>> result2 = Validator.advancedValidate(address2);
        String expected2 = "length less than 5";
        assertThat(result2.get("country")).contains(expected2);

        Address address3 = new Address("USA", null, null, null, "1");
        Map<String, List<String>> result3 = Validator.advancedValidate(address3);
        List<String> expected3 = List.of("city", "street", "houseNumber", "country");
        for (String field : expected3) {
            assertThat(result3).containsKey(field);
        }
        Address address4 = new Address("USA", null, null, null, "1");
        Map<String, List<String>> result4 = Validator.advancedValidate(address4);
        String expected4 = "length less than 5";
        assertThat(result2.get("country")).contains(expected4);
